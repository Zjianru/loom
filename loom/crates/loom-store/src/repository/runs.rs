use crate::{LoomStore, LoomStoreTx, RUNTIME_RUNS_DIR, load_json_row_from_conn};
...
impl LoomStoreTx<'_> {
    pub fn save_task_run(&mut self, run: &IsolatedTaskRun) -> Result<()> {
        self.maybe_fail("tx.save_task_run")?;
        let payload_json = serde_json::to_string(run).context("serializing task run")?;
        self.conn
            .execute(
                "
