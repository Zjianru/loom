...
这条链固定为：
1. 宿主语义层先在同 bundle 中给出：
   - `task_change`
   - `control_action(request_task_change)`
2. Loom 记录 `task_change_requested`
3. Loom 产出 `ChangeImpactAssessment`
4. 如变更被正式接受，再更新 `TaskScopeSnapshot`
5. 必要时重排 `PhasePlan`、重做 review、重算 acceptance
6. 必要时重评 `RiskAssessment`
